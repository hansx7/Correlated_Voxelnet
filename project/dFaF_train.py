...
from project.kittiDataLoader.dFaF_data_loader import KittiDetectionDataset
...
def train_iter(dataloader_train, model, optimizer, criterion,epoch, eval_index,
    ...
    conf_loss = 0.0
    reg_loss = 0.0
    for i_batch, (names, voxel_features_seq, pos_equal_one, neg_equal_one, targets) in enumerate(dataloader_train, 0):
        t0 = time.time()
        # wrapper to variable
...
def eval_iter(model, criterion, eval_index, writer, is_summary=True):
    ...
    datasets_val = KittiDetectionDataset(data_root, set='mini_val', train_type=cfg.train_type)
    dataloader_val = DataLoader(datasets_val, batch_size=cfg.batch_size, num_workers=cfg.n_cpu,
                                shuffle=True, drop_last=True)

    for i_batch, (names, voxel_features_seq, pos_equal_one, neg_equal_one, targets) \
...
def test_iter(dataloader, model, eval_path, data_root, epoch, writer, is_summary=True):
    ...
    for i_batch, (names, voxel_features_seq, pos_equal_one, neg_equal_one, targets, images) \
        ...
        # only using first frame
        prob = prob[:, :, :, :2]  # (batch_size, h, w, 2)
        reg_head = reg_head[:, :, :, :2, :]  # (batch_size, h, w, 2, 7)
        ...
        if is_summary and (i_batch + 1) % cfg.viz_interval == 0:
            ...
            front_image, bird_view, heatmap = \
                generate_summary_img(names[0], images[0], prob[0], pre_labels[0],
                                     scores[0], data_root, cfg, datasets='detection')
...
def train():
    ...
    # get dataset
    data_root = cfg.KITTI_DETECTION_DATASET_ROOT
    datasets_train = KittiDetectionDataset(data_root, set='train', train_type=cfg.train_type)
    ...
    # model.module.load_state_dict(torch.load('./checkpoints/'+ cfg.PROJECT_NAME +'/2019-01-05_16/79s.pkl'))

    # define optimizer
    # optimizer = optim.SGD(model.parameters(), lr=cfg.lr, momentum=0.9)
    optimizer = optim.Adam(model.parameters(), lr=cfg.lr)

    schedular = optim.lr_scheduler.StepLR(optimizer, step_size=cfg.lr_scheduler_step, gamma=0.3)
