...
def test_iter(dataloader, model, eval_path, data_root, epoch, writer, is_summary=True):
    ...
    for i_batch, (data_list, voxel_features_seq, pos_equal_one, neg_equal_one, targets, images) \
        ...
        for i in range(prob.size()[0]):
            ...
            names, pre_labels, confs = generate_tracking_pre_label(i_prob, i_reg_head,
                                                                   i_data_list, data_root, cfg)
...
def train():
    ...
    # model.module.load_state_dict(torch.load('./checkpoints/'+ cfg.PROJECT_NAME +'/2018-12-19_12/4s.pkl'))
    ...
    for i in range(cfg.epoches):
        ...
        if (i + 1) % cfg.test_interval == 0:
            ...
            os.makedirs(eval_root, exist_ok=True)
            # copy voxelnet_config.py and train.py to eval_root
