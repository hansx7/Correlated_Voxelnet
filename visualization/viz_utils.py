...
def draw_gt_boxes3d(gt_boxes3d, fig, box_id=0, color=(1, 1, 1), line_width=1, draw_text=True, text_scale=(1, 1, 1),
    ...
    for n in range(num):
        ...
        if draw_text:
            if text_type == 'name':
                mlab.text3d(b[4, 0], b[4, 1], b[4, 2], '%d' % name, scale=text_scale, color=txt_color, figure=fig)
            if text_type == 'scores':
                mlab.text3d(b[4, 0], b[4, 1], b[4, 2], '%.4f' % name, scale=text_scale, color=txt_color, figure=fig)
        for k in range(0, 4):
