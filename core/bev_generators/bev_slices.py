...
from core.bev_generators import bev_generator
...
class BevSlices(bev_generator.BevGenerator):
    ...
    def __init__(self, config):
        ...
        # Parse config
        self.height_lo = config.area_extents[2][0]
        self.height_hi = config.area_extents[2][1]
    ...
    def create_slice_filter(self, point_cloud, ares_extents, height_lo, height_hi):
        point_cloud = np.asarray(point_cloud)

        # Filter points within certain xyz range
        x_extents = ares_extents[0]
        y_extents = ares_extents[1]

        extents_filter = (point_cloud[0] > x_extents[0]) & \
                         (point_cloud[0] < x_extents[1]) & \
                         (point_cloud[1] > y_extents[0]) & \
                         (point_cloud[1] < y_extents[1]) & \
                         (point_cloud[2] > height_lo) & \
                         (point_cloud[2] < height_hi)
    ...
    def generate_bev(self,
        ...
        for slice_idx in range(self.num_slices):
            ...
            slice_filter = self.create_slice_filter(point_cloud, area_extents,
                                                     height_lo, height_hi)
        ...
        density_slice_filter = self.create_slice_filter(point_cloud,area_extents,
